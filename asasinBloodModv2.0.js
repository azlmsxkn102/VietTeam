// ==UserScript==
// @name       Assasin Blood Mod
// @namespace    -
// @version    2.0
// @description HAHA MOD TOP
// @author       VN
...
function animate(space, chance) {
    ...
    let characters;
    if(space) {
        characters = 'BEST HACK OVER!! ';
    } else {
        characters = 'VN Hacker';
    }
    if(space) {
...
//KeyCode event
    if (e.keyCode == 13 && chat == false) {
        chat = true;
    } else if (e.keyCode == 13 && chat == true) {
        chat = false;
    }
    if (e.keyCode == 96 && chat == false) {
        if (cek == true) {
            autoQ = false;
        }
        autoaim = true;
    ...
    if (e.keyCode == 27 && display == false) {
        e.preventDefault();
        document.getElementById("menu").style.display = "block";
        display = true;
    } else if (e.keyCode == 27 && display == true) {
        e.preventDefault();
        document.getElementById("menu").style.display = "none";
        display = false;
    }
    if (e.keyCode == 85 && chat == false) {
                if (cek == true) {
                autoQ = false;
            }
                autoaim = true
...
menu.id = "menu";
menu.style = "position: absolute; z-index:10; background-color:white; width: 40%; height:90%; top:5%; left:30%; border-radius:5px; display: none;";
menu.innerHTML = `
<div style="position:absolute; background-color:red; width:100%; height:auto; border-top-right-radius:5px; border-top-left-radius:5px; text-align:center;">
<b style="color:white; font-size:30px; padding-top:10px; padding-bottom:10px;"> Menu Version 1.0</b>
</div>
<div style="position:absolute; background-color:white; width:100%; height:10%; text-align:center; margin-top:10%;">
<span style="color:black; font-size:18px;">Features:<span><br />
<input id="autoQ" type="checkBox" />&nbsp;<b style="color:red; font-size:14px;">Auto Q - Beta Test!</b><br />
<button style="background-color:blue; color:white; padding:10px 20px;" id="autoBuy">Auto Buy</button>
</div>
`;
...
document.getElementById("").onclick = ()=>{
    ...
    if (check.checked == true) {} else {}
};
*/
document.getElementById("autoBuy").onclick = ()=>{
    doNewSend(["ch", ['Auto Buy']]);
    storeBuy(15);
    storeBuy(31);
    storeBuy(7);
    storeBuy(12);
...
document.getElementById("autoQ").onclick = ()=>{
    var check = document.getElementById("autoQ");
    if (check.checked == true) {
        doNewSend(["ch",['Auto Q: On']]);
        autoQ = true;
        cek = true;
    } else {
        doNewSend(["ch",['Auto Q: Off']]);
        autoQ = false;
        cek = false;
    }
};
...
//Skin Game
document.getElementById("gameName").innerText = "Assasin Blood Mod";
document.getElementById("gameName").style = "text-shadow: red 1px 1px 50px; font-size: 100px; color:red;";
var styles = document.createElement("style");
styles.type = "text/css";
styles.innerHTML = `
#setupCard, #guideCard{
background-color:red;
box-shadow: red 1px 1px 50px;
}
.menuCard {
...
#leaderboard, #foodDisplay, #woodDisplay, #stoneDisplay, #mapDisplay, #scoreDisplay, #itemInfoHolder, .storeTab, #storeHolder, #allianceHolder, #allianceInput, .allianceButtonM, #killCounter, #storeButton, #allianceButton{
color: red;
...
//enter button Skin
var speed = 700;
var color = new Array("#FF0066", "#0000FF", "#CCFF00", "#00FFCC", "#836FFF");
var countColor = 0;
...
setInterval(()=>{
    ...
    var rainbow = color[countColor];
document.getElementById("enterGame").style = "background-color:" + rainbow + ";";
    countColor+=1;
}, speed);
